...
class StatsWidget(QWidget):
    ...
    def _plot_hist_scatter(self):
        ...
        if continent_filter == "Tous":
            ...
            if dff_single.empty:
                ...
                self.ax.hist(dff_single[var].values, bins='auto', color="#0077c1",  # type: ignore
    ...
    def _analyze_histogram(self):
        ...
        ax2 = self.ax.twinx() 
        ax2.plot(xs, density(xs), color="#D2A7FE", linewidth=1.5, linestyle='-', label='Densité')# type: ignore
        ax2.set_yticks([])
        ...
        if not outliers_df.empty:
            ...
            self.ax.text(
                ...
                bbox=dict(boxstyle="square", facecolor="#FAFAFA", edgecolor='black', alpha=0.5)
            )
        else:
            self.ax.text(
